...
// the following cPath references come from application_top.php
  ...
  if ($category_depth == 'nested') {
    ...
    $category_query = tep_db_query("select cd.categories_name, cd.categories_description, c.categories_image, IF(cd.categories_htc_title_tag_alt != '',cd.categories_htc_title_tag_alt,cd.categories_htc_title_tag) as categories_htc_title_tag, cd.categories_htc_description from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . (int)$current_category_id . "' and cd.categories_id = '" . (int)$current_category_id . "' and cd.language_id = '" . (int)$languages_id . "'");
    /*** end Header Tags SEO ***/
...
// check to see if there are deeper categories within the current category
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
      ...
      echo '      <h5><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new) . '">' . $categories['categories_name'] .  $htsDesc . '</a></h5>';
      echo '    </div>';
      echo '  </div>';
      echo '</div>';
    }
