...
/*
  $Id: adwords_feed.php,
	v 3.0 2015/6/13 rework for osc 2.3.4 and prune lots of unused code
	v 2.0 2015/02/23 rewritten for Google shopping ads / custom labels and 
  adwords_feed.php,v 0.1 2012/07/23
  maintain adwords attributes for google products feed 
  ...
  Released under the GNU General Public License
	...
	******************************************************************
	...
	see https://support.google.com/merchants/answer/188494#customlabel
	
	Columns in product feed: 'custom label 0' to 'custom label 4'
	
	Can mean anything. Only one value per label per product. Up to 1000 unique values handled. Used for adding products to Ad groups in Campaigns
	...
	AdWords Exclusion Attributes: Exclude products from Product Ads
	excluded_destination
	
	Used to exclude selected offers from Shopping Ads. Can also be used to exclude if using on site search Google Commerce Search
	...
	*******************************************************************
	...
	Single table for custom labels - att to hold current meaning. 
	Link table for ad id - product id and attribute value
	Feed generator only adds first return if type set to label (set priority on group/label & order select in feed gen)
	This file is a page to manage all this:
		Insert / edit / delete label values
		Exclude products (don't show ones that aren't in feed)
...
//  require_once(DIR_WS_FUNCTIONS . 'qb_interface.php');
//  require(DIR_WS_LANGUAGES . $language . '/' . 'adwords_feed.php');
...
//  $filename = DIR_FS_CATALOG. DIR_WS_INCLUDES . 'qb_manager.php'; 
...
//	if (isset($_GET['active']) && !$mode_changed) {
...
/*	if (isset($_POST['order_id'])) {
  ...
  if (isset($_POST['update_button']) ) // adwords form confirmed
//  if (isset($_POST['action']) && ($_POST['action'] == 'insert' || $_POST['action'] == 'update')) // adwords form confirmed
  {
