...
*/
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'setflag':
        tep_db_query("update discount_codes set status = '" . (int)$HTTP_GET_VARS['flag'] . "' where discount_codes_id = '" . (int)$HTTP_GET_VARS['dID'] . "' limit 1");

        tep_redirect(tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page']));
        break;
      case 'insert':
        if (!empty($HTTP_POST_VARS['discount_codes']) && !empty($HTTP_POST_VARS['discount_values'])) {
          ...
          if ((int)$HTTP_POST_VARS['applies_to'] == 1) {
            if (isset($HTTP_POST_VARS['products_id']) && is_array($HTTP_POST_VARS['products_id']) && sizeof($HTTP_POST_VARS['products_id']) > 0) {
              $sql_data_array['products_id'] = implode(',', $HTTP_POST_VARS['products_id']);
              $error = false;
            }
          } elseif ((int)$HTTP_POST_VARS['applies_to'] == 2) {
            if (isset($HTTP_POST_VARS['categories_id']) && is_array($HTTP_POST_VARS['categories_id']) && sizeof($HTTP_POST_VARS['categories_id']) > 0) {
            ...
            if (isset($HTTP_POST_VARS['manufacturers_id']) && is_array($HTTP_POST_VARS['manufacturers_id']) && sizeof($HTTP_POST_VARS['manufacturers_id']) > 0) {
            ...
            $error = false;
          } elseif ((int)$HTTP_POST_VARS['applies_to'] == 6) {
          ...
          if ($error == false) {
            if (empty($HTTP_GET_VARS['dID'])) {
              ...
              tep_db_perform('discount_codes', $sql_data_array, 'update', "discount_codes_id = '" . (int)$HTTP_GET_VARS['dID'] . "'");
        ...
        break;
      case 'deleteconfirm':
        tep_db_query("delete from customers_to_discount_codes where discount_codes_id = '" . (int)$HTTP_GET_VARS['dID'] . "'");
        tep_db_query("delete from discount_codes where discount_codes_id = '" . (int)$HTTP_GET_VARS['dID'] . "' limit 1");
...
<?php
  if ($action == 'new') {
    ...
    if (isset($HTTP_GET_VARS['dID'])) {
      $discount_code_query = tep_db_query("select dc.discount_codes_id, dc.products_id, dc.categories_id, dc.manufacturers_id, dc.excluded_products_id, dc.customers_id, dc.orders_total, dc.shipping, dc.order_info, dc.exclude_specials, dc.discount_codes, dc.discount_values, dc.minimum_order_amount, dc.expires_date, dc.number_of_orders, dc.number_of_use, dc.number_of_products, dc.status from discount_codes dc where dc.discount_codes_id = '" . (int)$HTTP_GET_VARS['dID'] . "'");
...
function onload() {
  ...
  customers_onclick();
      ...
      <?php echo tep_draw_form('new_discount_code', 'discount_codes.php', (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=insert' . (isset($HTTP_GET_VARS['dID']) ? '&dID=' . (int)$HTTP_GET_VARS['dID'] : '')); ?><tr>
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td width="5"></td>
            <td class="main">
			<?php //echo '<label>' . str_replace('<input type="radio"', '<input onclick="applies_to_onclick();" type="radio"', tep_draw_radio_field('applies_to', '3', $dInfo->orders_total == 1)) . '&nbsp;' . TEXT_ORDER_TOTAL . '</label>'; ?>
			<?php echo '<label>' . str_replace('<input type="radio"', '<input onclick="applies_to_onclick();" type="radio"', tep_draw_radio_field('applies_to', '6', $dInfo->shipping == 2)) . '&nbsp;' . TEXT_SHIPPING . '</label>'; ?>
			</td>
          </tr>
          <tr>
    ...
    dateFormat: 'yy-mm-dd'
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="smallText" align="right" valign="top" nowrap><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('discount_codes.php', (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . (isset($HTTP_GET_VARS['dID']) ? 'dID=' . (int)$HTTP_GET_VARS['dID'] : ''))); ?></td>
          </tr>
        </table></td>
      </tr></form>
<?php
...
<?php
    ...
    while ($discount_codes = tep_db_fetch_array($discount_codes_query)) {
      ...
      if (isset($dInfo) && is_object($dInfo) && ($discount_codes['discount_codes_id'] == $dInfo->discount_codes_id)) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $dInfo->discount_codes_id) . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $discount_codes['discount_codes_id']) . '\'">' . "\n";
      }
?>
...
<?php
      if ($discount_codes['status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', 'Active', 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $discount_codes['discount_codes_id'] . '&action=setflag&flag=0') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', 'Set Inactive', 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $discount_codes['discount_codes_id'] . '&action=setflag&flag=1') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', 'Set Active', 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', 'Inactive', 10, 10);
      }
?></td>
                <td class="dataTableContent" align="right"><?php if (isset($dInfo) && is_object($dInfo) && ($discount_codes['discount_codes_id'] == $dInfo->discount_codes_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $discount_codes['discount_codes_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form('discount_codes', 'discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $dInfo->discount_codes_id . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $HTTP_GET_VARS['dID'])));
      break;
    default:
      if (is_object($dInfo)) {
        $heading[] = array('text' => '<strong>' . $dInfo->discount_codes . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $dInfo->discount_codes_id . '&action=new')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('discount_codes.php', 'page=' . $HTTP_GET_VARS['page'] . '&dID=' . $dInfo->discount_codes_id . '&action=delete')));
